...
model = load_model(
    '/content/drive/MyDrive/Colab Notebooks/Chatbot/model h5/chatbot_model.h5')
...
def getResponse(ints, intents_json):
    ...
    possible_responses = [i['responses']
                          for i in list_of_intents if i['tag'] == tag][0]
    history = Storage.old_answers[-len('possible_responses'):]
    print("** possible answers and history old answers",
          possible_responses, history)
    # list comprehension
    unused_answers = [
        answer for answer in possible_responses if answer not in history]
    print(unused_answers, " unused answers")
    unused_two = history[-(len(possible_responses)-1):]
    print(unused_two, 'last five answers')
    try:
        result = random.choice(
            [answer for answer in possible_responses if answer not in unused_two])
    except IndexError:
